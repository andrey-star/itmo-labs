...
public class G_Lcs {
	...
	public static void main(String[] args) throws IOException {
		...
		String st = s + "#" + t;
		in.close();
		Pair2 a = buildSufArray(st);
		int max = 0;
		String res = "";
		for (int i = 3; i < a.p.length; i++) {
			int curLength = st.length() - a.p[i];
			boolean curFirst = true;
			if (curLength > t.length()) {
				curFirst = false;
			}
			int prevLength = st.length() - a.p[i - 1];
			boolean prevFirst = true;
			if (prevLength > t.length()) {
				prevFirst = false;
			}
			if (prevFirst != curFirst) {
				if (max < a.lcp[i]) {
					max = a.lcp[i];
					res = st.substring(a.p[i], a.p[i] + max);
				}
			}
		}
