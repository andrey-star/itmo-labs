...
big_integer &big_integer::addsub(big_integer const &rhs, bool mode) {
    bool carry = false;
...
big_integer &big_integer::operator+=(big_integer const &rhs) {
    return addsub(rhs, true);
}

big_integer &big_integer::operator-=(big_integer const &rhs) {
    return addsub(rhs, false);
}
...
big_integer operator*(big_integer const &a, big_integer const &b) {
    ...
    big_integer b_abs = b.abs();
    if (a_abs > b_abs) {
        std::swap(a_abs.digits, b_abs.digits);
    }
    for (ui digit : a_abs.digits) {
        res += multiply_by_digit(b_abs, digit);
        b_abs.digits.insert(b_abs.digits.begin(), 0);
    }
    if (res_sign) {
        res = -res;
    }
